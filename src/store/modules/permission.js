import {asyncRoutes} from "../../routes/router";
...
const actions = {
  generateRoutes({ commit }, { roles, permissions }) {
    return new Promise(resolve => {
      let accessedRoutes;
      if (roles.includes('admin')) { // Toutes les routes si admin
        accessedRoutes = asyncRoutes || [];
      } else {
        accessedRoutes = filterAsyncRoutes(asyncRoutes, roles, permissions); // Filtrage des routes selon roles et permissions si pas admin
      }

      console.log('perm_routes', accessedRoutes);
